...
colors_start = [red, yellow, green,blue]
random.shuffle(colors_start)

colors = [colors_start[0], colors_start[1], colors_start[2]]
print(colors)
...
while capture.isOpened():
    ...
    if len(balls) == len(colors):
        ...
        for i in range(len(balls)):
            if prev_frame is not None and balls[i][0] < prev_frame:
                count += 1
            #print(count)
            if count == 3:
                you_win = True
            
            prev_frame = balls[i][0]
